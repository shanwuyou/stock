...
public:
	...
	int Connect(const char* ip,unsigned short port)
	{
		...
		if (SOCKET_ERROR == ret)
		{
			printf("<socket=%ld>错误，连接服务器<%s:%d>失败...\n",_sock, ip, port);
		}
		else {
			_isConnect = true;
...
#define RECV_BUFF_SZIE 10240
	...
	int RecvData(SOCKET cSock)
	{
		...
		if (nLen <= 0)
		{
			printf("<socket=%ld>与服务器断开连接，任务结束。\n", cSock);
			return -1;
		}
		//将收取到的数据拷贝到消息缓冲区
	...
	virtual void OnNetMsg(DataHeader* header)
	{
		switch (header->cmd)
		{
			...
			case CMD_ERROR:
			{
				printf("<socket=%ld>收到服务端消息：CMD_ERROR,数据长度：%d\n", _sock, header->dataLength);
			}
			break;
			default:
			{
				printf("<socket=%ld>收到未定义消息,数据长度：%d\n", _sock, header->dataLength);
			}
		}
	}
